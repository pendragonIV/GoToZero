...
public class LevelScene : MonoBehaviour
{
    ...
    private void PrepareLevels()
    {
        for (int i = 0; i < LevelManager.instance.levelData.GetLevels().Count; i++)
        {
            ...
            holder.GetChild(0).GetComponent<RectTransform>().DOAnchorPos(new Vector2(0, 0), .3f, false).SetEase(Ease.OutQuint).SetUpdate(true).SetDelay(.3f * i);
            holder.GetChild(0).GetComponent<CanvasGroup>().DOFade(1, .3f).SetUpdate(true).SetDelay(.3f * i);
        }
    }
}
